...
class UNet(nn.Module):
    def __init__(self, in_nc=1, out_nc=1, nc=[64, 128, 256, 512], nb=2, act_mode='R', 
        downsample_mode='strideconv', upsample_mode='convtranspose'):
        super(UNet, self).__init__()

        self.m_head = B.conv_layers(in_nc, nc[0], mode='C' + act_mode[-1])
        downsample_block = B.downsample_strideconv
        
        self.m_down1 = B.sequential(*[B.conv_layers(nc[0], nc[0], mode='C' + act_mode) for _ in range(nb)],
                                    downsample_block(nc[0], nc[1], mode='2' + act_mode))
        self.m_down2 = B.sequential(*[B.conv_layers(nc[1], nc[1], mode='C' + act_mode) for _ in range(nb)],
                                    downsample_block(nc[1], nc[2], mode='2' + act_mode))
        self.m_down3 = B.sequential(*[B.conv_layers(nc[2], nc[2], mode='C' + act_mode) for _ in range(nb)],
                                    downsample_block(nc[2], nc[3], mode='2' + act_mode))

        self.m_body  = B.sequential(*[B.conv_layers(nc[3], nc[3], mode='C' + act_mode) for _ in range(nb + 1)])

        upsample_block = B.upsample_convtranspose

        self.m_up3 = B.sequential(upsample_block(nc[3], nc[2], mode='2' + act_mode), *[B.conv_layers(nc[2], nc[2], mode='C' + act_mode) for _ in range(nb)])
        self.m_up2 = B.sequential(upsample_block(nc[2], nc[1], mode='2' + act_mode), *[B.conv_layers(nc[1], nc[1], mode='C' + act_mode) for _ in range(nb)])
        self.m_up1 = B.sequential(upsample_block(nc[1], nc[0], mode='2' + act_mode), *[B.conv_layers(nc[0], nc[0], mode='C' + act_mode) for _ in range(nb)])

        self.m_tail = B.conv_layers(nc[0], out_nc, bias=True, mode='C')
...
class ResUNet(nn.Module):
    def __init__(self, in_nc=1, out_nc=1, nc=[64, 128, 256, 512], nb=4, act_mode='L', downsample_mode='strideconv', upsample_mode='convtranspose'):
        ...
        self.m_head = B.conv_layers(in_nc, nc[0], bias=False, mode='C')
        ...
        self.m_tail = B.conv_layers(nc[0], out_nc, bias=False, mode='C')
...
class UNetResSubP(nn.Module):
    def __init__(self, in_nc=1, out_nc=1, nc=[64, 128, 256, 512], nb=2, act_mode='R', downsample_mode='strideconv', upsample_mode='convtranspose'):
        ...
        self.m_head = B.conv_layers(in_nc * sf * sf, nc[0], mode='C' + act_mode[-1])
        ...
        self.m_tail = B.conv_layers(nc[0], out_nc * sf * sf, bias=False, mode='C')
...
class UNetPlus(nn.Module):
    def __init__(self, in_nc=3, out_nc=3, nc=[64, 128, 256, 512], nb=1, act_mode='R', downsample_mode='strideconv', upsample_mode='convtranspose'):
        ...
        self.m_head = B.conv_layers(in_nc, nc[0], mode='C')
        ...
        self.m_down1 = B.sequential(*[B.conv_layers(nc[0], nc[0], mode='C' + act_mode) for _ in range(nb)], downsample_block(nc[0], nc[1], mode='2' + act_mode[1]))
        self.m_down2 = B.sequential(*[B.conv_layers(nc[1], nc[1], mode='C' + act_mode) for _ in range(nb)], downsample_block(nc[1], nc[2], mode='2' + act_mode[1]))
        self.m_down3 = B.sequential(*[B.conv_layers(nc[2], nc[2], mode='C' + act_mode) for _ in range(nb)], downsample_block(nc[2], nc[3], mode='2' + act_mode[1]))
        ...
        self.m_up3 = B.sequential(upsample_block(nc[3], nc[2], mode='2' + act_mode), *[B.conv_layers(nc[2], nc[2], mode='C' + act_mode) for _ in range(nb - 1)], B.conv_layers(nc[2], nc[2], mode='C' + act_mode[1]))
        self.m_up2 = B.sequential(upsample_block(nc[2], nc[1], mode='2' + act_mode), *[B.conv_layers(nc[1], nc[1], mode='C' + act_mode) for _ in range(nb - 1)], B.conv_layers(nc[1], nc[1], mode='C' + act_mode[1]))
        self.m_up1 = B.sequential(upsample_block(nc[1], nc[0], mode='2' + act_mode), *[B.conv_layers(nc[0], nc[0], mode='C' + act_mode) for _ in range(nb - 1)], B.conv_layers(nc[0], nc[0], mode='C' + act_mode[1]))

        self.m_tail = B.conv_layers(nc[0], out_nc, mode='C')
...
class NonLocalUNet(nn.Module):
    def __init__(self, in_nc=3, out_nc=3, nc=[64,128,256,512], nb=1, act_mode='R',
        ...
        self.m_down1 = B.sequential(*[B.conv_layers(nc[0], nc[0], mode='C' + act_mode) for _ in range(nb)], downsample_block(nc[0], nc[1], mode='2' + act_mode))
        self.m_down2 = B.sequential(*[B.conv_layers(nc[1], nc[1], mode='C' + act_mode) for _ in range(nb)], downsample_block(nc[1], nc[2], mode='2' + act_mode))
        self.m_down3 = B.sequential(down_nonlocal, *[B.conv_layers(nc[2], nc[2], mode='C' + act_mode) for _ in range(nb)], downsample_block(nc[2], nc[3], mode='2' + act_mode))
        ...
        self.m_up3 = B.sequential(upsample_block(nc[3], nc[2], mode='2' + act_mode), *[B.conv_layers(nc[2], nc[2], mode='C' + act_mode) for _ in range(nb)], up_nonlocal)
        self.m_up2 = B.sequential(upsample_block(nc[2], nc[1], mode='2' + act_mode), *[B.conv_layers(nc[1], nc[1], mode='C' + act_mode) for _ in range(nb)])
        self.m_up1 = B.sequential(upsample_block(nc[1], nc[0], mode='2' + act_mode), *[B.conv_layers(nc[0], nc[0], mode='C' + act_mode) for _ in range(nb)])
...
if __name__ == '__main__':
    x = torch.ones(1, 4, 128, 128)
    netG = UNetRes(in_nc=4, out_nc=3, nc=[64, 128, 256, 512], nb=4, act_mode='R',
                   downsample_mode="strideconv", upsample_mode="convtranspose")
