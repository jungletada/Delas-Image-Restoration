...
def r2c(x):
    ...
    """0: add by channel，1: add by row，2: add by column"""
...
def cmul(t1, t2):
    ...
    Args:
        t1: N x C x H x W x 2, complex tensor
        t2: N x C x H x W x 2
    Returns:
        output: N x C x H x W x 2
    '''
...
def pre_calculate(x, k, sf):
    '''
    Args:
        x: N x C x H x W, LR input
        k: N x C x h x w, kernel
        sf: integer, scale factor
    Returns:
        FB, FBC, F2B, FBFy will be reused during iterations
    '''
    w, h = x.shape[-2:]
    FB = p2o(k, (w * sf, h * sf))
